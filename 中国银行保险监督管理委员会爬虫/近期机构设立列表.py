...
base_url = 'http://xkz.cbirc.gov.cn/jr/getLicence.do?useState=1'
...
def get_detail(url, jianyan):
    ...
    if not insti or not _id:
        print(r.url)
        print(r.text)
    return address.strip(), at.strip(), brief.strip(), insti.strip(), _id.strip()


index = 2
for item in range(0, 3155, 10):
    ...
    while True:
        time.sleep(0.1)
        try:
            r = requests.post(url=base_url, data=data, proxies=proxies, headers=headers, timeout=2)
            datas = r.json()['datas']
        except Exception as e:
            print(e)
            print('列表页爬取异常')
