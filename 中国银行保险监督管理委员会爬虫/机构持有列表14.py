...
def get_detail(url, jianyan):
    while True:
        try:
            ...
            if '请点击机构名称查看机构详细信息！' in r.text:
                with open('problems_urls.txt', 'a', encoding='utf8') as af:
                    af.write(r.url + '\n')
                return [' '] * 9
...
with open('机构持有列表14.csv', 'w', encoding='utf-8-sig', newline='') as wf:
    writer = csv.writer(wf)
    writer.writerow([['机构编码', '机构名称', '批准成立日期', '发证日期', '机构地址', '机构所在地', '机构简称', '发证机关', '流水号', '前流水号', '前机构编码', '前机构名称', '前机构地址']])
index = 2
for item in range(125000, 150000, 10):
    ...
    for one_data in datas:
        ...
        with open('机构持有列表14.csv', 'a', encoding='utf-8-sig', newline='') as af:
